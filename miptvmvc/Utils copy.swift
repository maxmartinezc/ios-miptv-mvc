...
struct Utils {
    ...
    static func setUserFavorites(data: Favorite) {
        let encoder = JSONEncoder()
        if let encoded = try? encoder.encode(data) {
            UserDefaults.standard.set(encoded, forKey: K.UserConfig.favoriteList)
        }
    }
    
    static func getUserFavorites() -> Favorite? {
        guard let data = UserDefaults.standard.data(forKey: K.UserConfig.favoriteList) else { return nil }
        if let favoriteList = try? JSONDecoder().decode(Favorite.self, from: data) {
            return favoriteList;
        }
        return nil
    }
